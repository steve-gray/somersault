...
class Container {
  ...
  _resolveInternal(tag, depth, built, resolverContext) {
    ...
    // If we've got a parent, fall back to the parent
    if (this._parent) {
      this._debug('%sCannot find tag in container, checking parent.', spacer);
      return this._parent._resolveInternal(tag, depth + 1, built, resolveRoot);
    }
