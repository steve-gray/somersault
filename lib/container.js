...
class Container {
    ...
    if (typeof value === 'function') {
      ...
      if (parserHelpers.isArrowFunction(parsed) || parserHelpers.isFunction(parsed)) {
        ...
        generatorDef = (container, depth, context) => {
          ...
          const args = [];
          if (parserHelpers.isArrowFunction(parsed)) {
            args.push(null);
          }
          ...
          console.log('building with args: ' + JSON.stringify(args));
  ...
  build(generator) {
    ...
    const argList = parserHelpers.getParameters(generator);
    const args = [];
