...
class Container {
   ...
   * @returns                   - Container that is filtering out specified tags
   */
  filterOut(tags) {
    this._debug('filterOut(%s)', tags);
    if (!tags) {
      throw new Error('Require a tags list for filterOut');
    }

    // Create the filter set.
    const filterSet = Array.isArray(tags) ? tags : [tags];
    return new Container(this, {
      filterOutTags: filterSet,
    });
  }
  ...
  _getResolveCandidatesInternal(tag) {
    ...
    // If we've got filtering tags, then filter this list of registrations
    // and remove filters.
    if (this._options.filterOutTags && this._options.filterOutTags.length > 0) {
      return registrations.filter((registration) => {
        for (const tagToExclude of this._options.filterOutTags) {
